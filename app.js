...
• [Paste your study notes here for a summary]">${this._esc(this.wizardData.topic)}</textarea>
  ...
  async _sendDirect(text, lang, depth, style, tool) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        // Pass BOTH the friendly summary and the real technical reason —
        // the error card shows the friendly text plus a collapsible
        // "technical details" section with the actual model/HTTP errors,
        // so issues can be diagnosed without needing server log access.
        this._showState('error', friendly, err.message);
        this._toast('error', 'fa-exclamation-circle', friendly);
        this._showToolbar(false);
      }
    } finally {
      this.generating = false;
  ...
  _showState(state, errMsg, techDetails) {
    if (this.el.emptyState)   this.el.emptyState.style.display   = 'none';
    if (this.el.thinkingWrap) this.el.thinkingWrap.style.display = 'none';
    if (this.el.resultArea)   this.el.resultArea.style.display   = 'none';
    ...
    switch (state) {
      ...
      case 'error':
        if (this.el.resultArea) {
          this.el.resultArea.style.display = 'block';
          // Only show technical details if they add real info beyond the friendly message
          const hasDetails = techDetails && techDetails.trim() && techDetails !== errMsg;
          this.el.resultArea.innerHTML = `
            <div class="error-card">
              ...
              <details class="error-tech-details">
                <summary><i class="fas fa-bug"></i> Technical details (for debugging)</summary>
                <pre>${this._esc(techDetails)}</pre>
              </details>` : ''}
