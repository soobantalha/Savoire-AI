'use strict';
/* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
   SAVOIRÉ AI v2.0 — ULTIMATE FRONTEND (app.js) — 8500+ LINES
...
class SavoireApp {
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 4: STREAK & ANALYTICS SYSTEM
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _updateAnalyticsDisplay() {
    ...
    if (this.elements.sidebarLastActive) {
      if (this.lastActive) {
        ...
        else if (this.lastActive === new Date(Date.now() - 86400000).toISOString().split('T')[0]) this.elements.sidebarLastActive.textContent = 'Yesterday';
  ...
  _boot() {
    ...
    console.log(`%cAll IDs Matched | 8500+ Lines | Fully Functional`, 'color:#00ff88;font-size:10px');
  }
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 8: WIZARD SYSTEM (COMPLETE 5-STEP WIZARD)
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _renderWizardStep() {
    ...
    if (wizardBody) {
      switch (this.wizardStep) {
        ...
        case 3: wizardBody.innerHTML = this._renderWizardDepthStyleStep(); this._bindWizardDepthStyleClicks(); break;
        case 4: wizardBody.innerHTML = this._renderWizardReviewStep(); break;
      }
    }
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 9: CORE GENERATION (LIVE STREAMING)
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  async _send() {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        this._hideStreamOverlay();
        this._showState('error', err.message || 'Something went wrong.');
  ...
  _mobileScrollToOutput() {
    ...
    this.elements.rightPanel?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  ...
  async _streamSSE(message, opts) {
    return new Promise((resolve, reject) => {
      ...
      fetch(SAVOIRÉ.API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body, signal: this.streamCtrl?.signal })
        .then(async res => {
          if (!res.ok) { reject(new Error(`Server error (${res.status})`)); return; }
          const ct = res.headers.get('content-type') || '';
          if (!ct.includes('text/event-stream')) { const data = await res.json(); if (data.error) reject(new Error(data.error)); else this._simulateStream(data, resolve, reject); return; }
              ...
              while (true) {
                ...
                for (const line of lines) {
                  ...
                  try {
                    ...
                    else if (evt.topic !== undefined) { if (this.elements.sfpText) { this.elements.sfpText.classList.remove('live-md'); this.elements.sfpText.classList.add('done'); } resolve(evt); return; }
  ...
  async _simulateStream(data, resolve, reject) {
    ...
    const tick = () => {
      ...
      if (i >= notesText.length) { if (this.elements.sfpText) { this.elements.sfpText.classList.remove('live-md'); this.elements.sfpText.classList.add('done'); } resolve(data); return; }
      this.streamBuffer += notesText.slice(i, i + chunkSize);
      i += chunkSize;
      if (this.elements.sfpText) { try { this.elements.sfpText.innerHTML = this._renderMdLive(this.streamBuffer); this.elements.sfpText.classList.add('live-md'); } catch(e) { this.elements.sfpText.textContent = this.streamBuffer; } if (this.elements.sfpScroll) this.elements.sfpScroll.scrollTop = this.elements.sfpScroll.scrollHeight; }
    ...
    switch (state) {
      ...
      case 'error':
        if (this.elements.resultArea) {
          ...
          this.elements.resultArea.innerHTML = `<div class="error-card"><div class="error-card-hdr"><i class="fas fa-exclamation-circle"></i> Generation Failed</div><div class="error-card-body">${this._esc(errorMsg)}</div><div class="error-card-hint">The AI models may be temporarily busy. Please try again.</div><button class="btn btn-primary" style="margin-top:16px" onclick="document.getElementById('mainInput').focus()"><i class="fas fa-redo"></i> Try Again</button></div>`;
          this._scrollOutArea();
        }
        break;
      default:
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 11: RESULT RENDERING (NOTES, FLASHCARDS, QUIZ, SUMMARY, MIND MAP)
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _fcNav(dir) {
    ...
    if (this.elements.fcFront) this.elements.fcFront.textContent = card.q;
    if (this.elements.fcBack) this.elements.fcBack.innerHTML = this._renderMd(card.a);
  ...
  _buildQuizHTML(data) {
    ...
    this.quizData = qs.map((q,idx) => {
      let options = this._generateMCQOptions(q, data, idx);
      return { question: q.question, answer: q.answer || '', options, answered: false, correct: false, selectedIdx: -1 };
    });
  ...
  _generateMCQOptions(qa, data, idx) {
    ...
    const correctShort = this._stripMd(correctAnswer).split('.')[0].trim().substring(0,100);
    const genericFallbacks = ['This is not correct', 'This is a common misconception', 'This applies to a different concept', 'None of the above'];
    let opts = [{ text: correctShort, isCorrect: true }, ...genericFallbacks.slice(0,3).map(d => ({ text: d, isCorrect: false }))];
  ...
  _quizSelectOption(qIdx,optIdx) {
    ...
    else { this._toast('info','fa-book-open','✗ Not quite — check the answer below'); }
    ...
    if(optsContainer){ const btns=optsContainer.querySelectorAll('.quiz-opt-btn'); btns.forEach((btn,oi)=>{ btn.disabled=true; if(q.options[oi].isCorrect) btn.classList.add('correct'); else if(oi===optIdx && !q.options[oi].isCorrect) btn.classList.add('wrong'); }); }
  ...
  _buildSummaryHTML(data) {
    ...
    if(data.ultra_long_notes){
      ...
      const tldr=raw.split('\n\n')[0] || `Overview of ${data.topic || 'the topic'}.`;
  ...
  _buildMindmapHTML(data) {
    ...
    const branchHtml = branches.map(b => `<div class="mm-branch"><div class="mm-branch-hdr" style="color:${b.color}"><i class="fas fa-project-diagram"></i> ${this._esc(b.name)}</div><div class="mm-nodes-list">${b.items.map(item=>`<div class="mm-node"><span class="mm-node-dot" style="background:${b.color}"></span><span class="mm-node-text">${this._esc(item)}</span></div>`).join('')}</div></div>`).join('');
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 12: PROFESSIONAL PDF GENERATION
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _downloadPDF() {
    ...
    try {
      ...
      const drawPageFooter = (pgNum, pgTotal) => {
        ...
        doc.setFontSize(6.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(155, 140, 118);
      ...
      const writeText = (text, fontSize, bold, color, indent=0, lineH=1.6) => {
        ...
        let used=0;
        lines.forEach(line=>{ checkSpace(lh+1); doc.text(line, ml+indent, y); y+=lh; used+=lh; });
        return used;
      };
      ...
      doc.setFontSize(16); doc.setTextColor([18,12,4]); doc.text('Think Less. Know More.', pw/2, y+35, {align:'center'});
      doc.setFontSize(12); doc.setTextColor([55,48,38]); doc.text(`Study Notes: ${data.topic || 'Topic'}`, pw/2, y+55, {align:'center'});
      y += 90;
      ...
      if(data.ultra_long_notes){
        ...
        paragraphs.forEach(para=>{
          ...
          if(!trimmed) return;
          if(/^#{1,4} /.test(trimmed)){ const headText=trimmed.replace(/^#+\s*/,''); checkSpace(12); y+=3; writeText(headText,14,true,[180,150,40],0,1.3); y+=2; }
          else if(trimmed.startsWith('- ')||trimmed.startsWith('* ')){ const items=trimmed.split('\n').filter(Boolean); items.forEach(item=>{ writeText(item.replace(/^[-*]\s+/,''),10,false,[18,12,4],10,1.6); }); }
          else{ writeText(trimmed,10,false,[18,12,4],0,1.65); y+=2.5; }
        });
      }
    ...
    } catch(err){ this._toast('error','fa-times',`PDF failed: ${err.message}`); }
  }

  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 13: COPY, SAVE, SHARE, CLEAR, HISTORY, SAVED NOTES
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _saveNote() {
    ...
    this.saved.unshift(note); this._save('sv_saved',this.saved); this._updateHeaderStats(); this._updateAnalyticsDisplay(); this._toast('success','fa-star',`Saved!`);
  }
  ...
  _clearOutput() {
    ...
    this._confirm('Clear the current output?',()=>{ this.currentData=null; this._showState('empty'); this.fcCards=[]; this.quizData=[]; this._toast('info','fa-trash','Output cleared.'); });
  }
  ...
  _renderSidebarHistory() {
    ...
    if(!this.history.length){ this.elements.lpHistList.innerHTML = '<div class="lp-hist-empty">No history yet.</div>'; return; }
    ...
    this.elements.lpHistList.innerHTML = this.history.slice(0,6).map(h=>`<div class="lp-hist-item" onclick="window._app._loadHistoryItem('${h.id}')"><i class="fas ${ICONS[h.tool]||'fa-book'} lp-hist-icon"></i><div class="lp-hist-topic">${this._esc((h.topic||'').substring(0,32))}</div><div class="lp-hist-time">${this._relTime(h.ts)}</div><button class="lp-hist-delete" onclick="event.stopPropagation();window._app._deleteHistory('${h.id}')"><i class="fas fa-times"></i></button></div>`).join('');
  }
  ...
  /* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
     SECTION 14: SETTINGS, THEME, SIDEBAR, MOBILE, KEYBOARD SHORTCUTS
     ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */
  ...
  _openSettingsModal(){
    ...
    if (this.elements.dsStats){
      ...
      this.elements.dsStats.innerHTML=`<div class="ds-stat"><span class="ds-val">${this.history.length}</span><div class="ds-lbl">History</div></div><div class="ds-stat"><span class="ds-val">${this.saved.length}</span><div class="ds-lbl">Saved</div></div><div class="ds-stat"><span class="ds-val">${this.sessions}</span><div class="ds-lbl">Sessions</div></div><div class="ds-stat"><span class="ds-val">${totalKB}KB</span><div class="ds-lbl">Storage</div></div><div class="ds-stat"><span class="ds-val">${this.totalWordsGenerated.toLocaleString()}</span><div class="ds-lbl">Words</div></div>`;
    }
    this._openModal('settingsModal');
  }
  
  _saveName(){ const name = this.elements.nameInput?.value?.trim(); if(!name||name.length<2){ this._toast('error','fa-times','Name must be at least 2 characters.'); return; } this.userName=name; localStorage.setItem('sv_user',name); this._updateUserUI(); this._toast('success','fa-check','Name updated!'); }
  ...
  _importDataJson(file){
    ...
    reader.onload=e=>{
      try{
        ...
        this._updateUserUI();
        this._updateAnalyticsDisplay();
        this._toast('success','fa-check','Backup restored!');
  ...
  _handleFile(file){ 
    ...
    if(file.size>200000){ this._toast('error','fa-times','File too large. Max 200KB.'); return; }
    const reader=new FileReader();
    reader.onload=e=>{
  ...
  showDemo(){
    const demoOverlay = this._el('demoOverlay');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('%c✨ Savoiré AI v2.0 — Fully Loaded (8500+ Lines)', 'color:#d4af37;font-size:14px;font-weight:bold');
  console.log('%cAll IDs Matched | All Features Working | Ready to Use', 'color:#00ff88;font-size:12px');
});

/* ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
   END OF FILE — app.js v2.0 (8500+ LINES)
   ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ */