...
class SavoireApp {
  ...
  // Resolve the correct display name: saved name → Firebase auth → 'Scholar'
  _resolveUserName() {
    try {
      const local = localStorage.getItem('sv_user');
      if (local && String(local).trim()) return String(local).trim();
      const u = window.firebaseAuthInstance?.currentUser;
      if (u) {
  ...
  _updateUserUI() {
    if (!this.userName) this.userName = this._resolveUserName();
...
• [Paste your study notes here for a summary]">${this._esc(this.wizardData.topic)}</textarea>
  ...
  async _sendDirect(text, lang, depth, style, tool, counts) {
    ...
    try {
      ...
      // Update credits display with the EXACT remaining balance from the server
      if (data && data._tokens_remaining !== undefined && window._updatePaidTokenBar) {
        const limit = (window.PAID_TOKENS && window.PAID_TOKENS.limit) || null;
        window._updatePaidTokenBar(data._tokens_remaining, limit);
        window.PAID_REMAINING = data._tokens_remaining;
      }
    ...
    const header = `
      <div class="result-hdr">
        <div class="rh-left">
          ...
          ${ (data._tokens_used_this_request || data._tokens_remaining) ? `<div class="credit-result-banner" style="margin-top:12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:10px 14px;background:linear-gradient(135deg,rgba(34,211,238,.10),rgba(25,112,255,.08));border:1px solid rgba(34,211,238,.25);border-radius:12px;font-size:.78rem;color:#c9cfdd"><span style="display:inline-flex;align-items:center;gap:6px"><i class="fas fa-coins" style="color:#22d3ee"></i> Credits Used: <b style="color:#22d3ee">${(data._tokens_used_this_request||0).toLocaleString()}</b></span><span style="opacity:.4">|</span><span>Remaining: <b style="color:#00ff88">${(data._tokens_remaining||0).toLocaleString()}</b></span><span style="margin-left:auto;display:flex;gap:8px"><a href="#" onclick="event.preventDefault(); window.openBuyCreditsModal && window.openBuyCreditsModal()" style="padding:4px 10px;background:linear-gradient(135deg,#22d3ee,#1970ff);border-radius:999px;color:#fff;text-decoration:none;font-weight:700;font-size:.7rem">Buy More</a><a href="#" onclick="event.preventDefault(); window.openCreditHistory && window.openCreditHistory()" style="padding:4px 10px;background:rgba(255,255,255,.06);border-radius:999px;color:#aab2cc;text-decoration:none;font-size:.7rem">History</a></span></div>` : '' }
    ...
    const exportBar = `
      <div class="export-bar">
        <button class="exp-btn pdf" onclick="window._app._downloadPDF()">
          <i class="fas fa-file-pdf"></i><span>Download</span>
        </button>
  ...
  async _downloadPDF(mode = null) {
    ...
    const hasLiveNotes = !!(data._live_notes_buffer && data._live_notes_buffer.length > 50);
    if (!mode && hasLiveNotes) {
      this._showDownloadChoiceModal();
      return;
    }
    ...
    if (mode === 'live') {
      const liveData = {
        ...data,
        topic: `${data.topic || 'Study Material'} — Live Notes`,
        ultra_long_notes: data._live_notes_buffer || data.ultra_long_notes || '',
        flashcards: [],
        quiz_questions: [],
        mindmap: null,
        key_concepts: [],
  ...
  _showDownloadChoiceModal() {
    const existing = document.getElementById('downloadChoiceModal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'downloadChoiceModal';
    modal.className = 'download-choice-overlay';
    modal.innerHTML = `
      <div class="download-choice-modal">
        <button class="download-choice-close" aria-label="Close" onclick="document.getElementById('downloadChoiceModal')?.remove()">
          <i class="fas fa-times"></i>
        </button>
        <div class="download-choice-badge">PREMIUM DOWNLOAD</div>
        <h3>Choose what to download</h3>
        <p>Download the polished final output or the original live notes stream captured during generation.</p>
        <div class="download-choice-grid">
          <button class="download-choice-card final" onclick="document.getElementById('downloadChoiceModal')?.remove(); window._app._downloadPDF('final');">
            <div class="download-choice-icon"><i class="fas fa-gem"></i></div>
            <div class="download-choice-title">Final Output PDF</div>
            <div class="download-choice-desc">Beautiful formatted export with your polished result and premium layout.</div>
          </button>
          <button class="download-choice-card live" onclick="document.getElementById('downloadChoiceModal')?.remove(); window._app._downloadPDF('live');">
            <div class="download-choice-icon"><i class="fas fa-bolt"></i></div>
            <div class="download-choice-title">Live Notes PDF</div>
            <div class="download-choice-desc">Download the original live stream notes exactly as they were generated.</div>
          </button>
        </div>
      </div>`;
  ...
  _saveNote() {
    ...
    // Max saved: 30 — auto-remove oldest entries beyond the cap
    if (this.saved.length > SAVOIRÉ.MAX_SAVED) {
      const removed = this.saved.slice(SAVOIRÉ.MAX_SAVED);
      this.saved = this.saved.slice(0, SAVOIRÉ.MAX_SAVED);
      removed.forEach(r => { try { this._deleteSavedFromCloud(r.id); } catch(e){} });
    }
  ...
  _addHistory(item) {
    ...
    // Max history: 30 — auto-remove oldest entries beyond the cap
    if (this.history.length > SAVOIRÉ.MAX_HISTORY) {
      const removed = this.history.slice(SAVOIRÉ.MAX_HISTORY);
      this.history = this.history.slice(0, SAVOIRÉ.MAX_HISTORY);
      removed.forEach(r => { try { this._deleteHistoryFromCloud(r.id); } catch(e){} });
    }
  ...
  // ─── DELETE ACCOUNT 3-STEP (Prevent exploit) ───────────────────────────────
  _deleteAccountFlow() {
    const step1 = () => {
      this._confirm('⚠️ Delete Account? This will delete ALL your credits, history, saved notes and cannot be undone. Are you sure?', () => step2());
    };
    const step2 = () => {
      // Create custom modal for typing DELETE
      const existing = document.getElementById('deleteStep2Modal');
      if (existing) existing.remove();
      const modal = document.createElement('div');
      modal.id = 'deleteStep2Modal';
      modal.className = 'modal-overlay';
      modal.style.display = 'flex';
      modal.style.zIndex = '20000';
      modal.innerHTML = `
        <div class="modal modal-sm" style="max-width:400px">
          <div class="modal-header"><div class="modal-title" style="color:#ff4444"><i class="fas fa-exclamation-triangle"></i> Type DELETE to confirm</div><button class="modal-close" onclick="document.getElementById('deleteStep2Modal').remove()"><i class="fas fa-times"></i></button></div>
          <div class="modal-body">
            <p style="color:#aab2cc;font-size:.85rem;margin-bottom:12px">This action is irreversible. Type <b style="color:#ff4444">DELETE</b> in capital letters to confirm.</p>
            <input id="deleteConfirmInput" type="text" placeholder="Type DELETE" style="width:100%;padding:10px 12px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;margin-bottom:12px">
            <div style="display:flex;gap:8px">
              <button id="deleteStep2Btn" style="flex:1;padding:10px;background:#ff4444;color:#fff;border:none;border-radius:10px;font-weight:700;opacity:.5" disabled>Confirm DELETE</button>
              <button onclick="document.getElementById('deleteStep2Modal').remove()" style="padding:10px 16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#aab2cc;border-radius:10px">Cancel</button>
            </div>
          </div>
        </div>
      `;
      ...
      input?.addEventListener('input', () => {
        if (input.value === 'DELETE') { btn.disabled = false; btn.style.opacity = '1'; }
    ...
    const step3 = () => {
      const modal = document.createElement('div');
      modal.id = 'deleteStep3Modal';
      modal.className = 'modal-overlay';
      modal.style.display = 'flex';
      modal.style.zIndex = '20000';
      modal.innerHTML = `
        <div class="modal modal-sm" style="max-width:400px">
          <div class="modal-header"><div class="modal-title" style="color:#ff4444">Final Confirmation</div><button class="modal-close" onclick="document.getElementById('deleteStep3Modal').remove()"><i class="fas fa-times"></i></button></div>
          <div class="modal-body">
            <p style="color:#ffae00;font-size:.85rem;margin-bottom:12px">⚠️ Last chance! Your <b>${(window.PAID_TOKENS?.remaining||window.PAID_REMAINING||0).toLocaleString()} credits</b> will be lost forever.</p>
            <p style="color:#aab2cc;font-size:.8rem;margin-bottom:16px">Re-authenticate to delete account. Enter your password if you signed up via Email, or just click Delete if you used Google.</p>
            <input id="deletePasswordInput" type="password" placeholder="Enter password (if email login)" style="width:100%;padding:10px 12px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;margin-bottom:12px">
            <div style="display:flex;gap:8px">
              <button id="deleteFinalBtn" style="flex:1;padding:10px;background:linear-gradient(135deg,#ff4444,#cc0000);color:#fff;border:none;border-radius:10px;font-weight:700">Yes, Delete My Account Forever</button>
              <button onclick="document.getElementById('deleteStep3Modal').remove()" style="padding:10px 16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#aab2cc;border-radius:10px">Cancel</button>
            </div>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      document.getElementById('deleteFinalBtn')?.addEventListener('click', async () => {
        try {
          // Try Firebase delete
          const { getAuth, deleteUser, reauthenticateWithCredential, EmailAuthProvider } = await import("https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js");
          const auth = getAuth();
          ...
          if (pwd && user.email) {
            ...
            } catch(e) { console.log('Reauth failed, trying delete anyway', e.message); }
          }
          // Delete Firestore data via API
          try {
            const token = localStorage.getItem('sv_firebase_token');
          ...
          alert('Delete failed: ' + err.message + '. Try logout and login again.');
          console.error(err);
        }
      });
    };
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Sync the display name from Firestore if we have none locally
          if (!this.userName && data.displayName && String(data.displayName).trim()) {
            this.userName = String(data.displayName).trim();
            localStorage.setItem('sv_user', this.userName);
            this._updateUserUI();
          }
  ...
  // ─── CREDIT HISTORY (Firestore-synced) ────────────────────────────────────
  async _openCreditHistory() {
    const modal = this._el('creditHistoryModal');
    if (modal) {
      modal.style.display = 'flex';
      modal.style.zIndex = modal.style.zIndex || '12000';
    }
    const listEl = this._el('creditHistoryList');
    if (!listEl) return;
    listEl.innerHTML = '<div style="text-align:center;padding:40px;color:rgba(255,255,255,.4)">Loading history...</div>';
    const tok = this._getFbToken();
    if (!tok) {
      listEl.innerHTML = '<div style="text-align:center;padding:20px;color:#ffae00">Please login to view credit history.</div>';
      return;
    }
    try {
      ...
      console.error('Credit history fetch failed', e);
      listEl.innerHTML = `<div style="color:#ff6b6b;padding:20px;text-align:center">Could not load credit history.<br><span style="font-size:.75rem;color:#aab2cc">${this._esc(String(e.message||e))}</span></div>`;
    }
  }
  ...
  _renderCreditHistoryModal(data) {
    const listEl = this._el('creditHistoryList');
    if (!listEl) return;
    const items = this.creditHistory || [];
    const filter = this._qs('.hist-filter.active[data-ch-filter]')?.dataset?.chFilter || 'all';
    let filtered = items;
    if (filter === 'generation') filtered = items.filter(i => i.type === 'generation');
    if (filter === 'purchase')   filtered = items.filter(i => i.type === 'purchase');
    if (filter === 'bonus')      filtered = items.filter(i => i.type === 'bonus');

    if (!filtered.length) {
      listEl.innerHTML = '<div style="text-align:center;padding:30px;color:rgba(255,255,255,.4)">No history yet</div>';
    } else {
      listEl.innerHTML = filtered.map(it => {
        const isPos = (it.creditsChange || 0) > 0;
        const icon  = it.icon || (it.type === 'purchase' ? '💳' : it.type === 'bonus' ? '🎁' : '📝');
        const color = it.type === 'purchase' ? 'rgba(34,211,238,.10)' : it.type === 'bonus' ? 'rgba(0,255,136,.10)' : 'rgba(255,255,255,.04)';
        const time  = it.timestamp ? new Date(it.timestamp).toLocaleString() : '';
        return `<div class="credit-history-item" style="background:${color}">
          <div class="credit-history-icon">${icon}</div>
          <div class="credit-history-info">
            <div class="credit-history-title">${this._esc(it.description || it.topic || 'Activity')}</div>
            <div class="credit-history-sub">${this._esc(it.topic || it.tool || '')}${time ? ' • ' + this._esc(time) : ''}</div>
          </div>
          <div style="text-align:right">
            <div class="credit-history-amount ${isPos ? 'positive' : 'negative'}">${isPos ? '+' : ''}${(it.creditsChange || 0).toLocaleString()}</div>
            <div class="credit-history-remaining">Rem: ${(it.creditsRemaining ?? 0).toLocaleString()}</div>
          </div>
        </div>`;
      }).join('');
    }

    const sumEl = this._el('creditHistorySummary');
    if (sumEl && data) {
      const total = data.totalPurchased || 0;
      const used  = data.totalUsed || 0;
      const rem   = data.balance ?? 0;
      sumEl.innerHTML = `<span>Total Purchased: ${total.toLocaleString()}</span><span>Used: ${used.toLocaleString()}</span><span>Remaining: ${rem.toLocaleString()}</span>`;
    }
  }
  ...
  async _fetchCloudSaved() {
    ...
    if (addedFromCloud>0 || pushedToCloud>0) {
      ...
      if (this.saved.length > SAVOIRÉ.MAX_SAVED) this.saved = this.saved.slice(0, SAVOIRÉ.MAX_SAVED);
      this._save('sv_saved', this.saved);
      this._renderSidebarSaved();
      this._updateAllStats();
  ...
  _bindAll() {
    ...
    this._qsa('.hist-filter').forEach(b => {
      if (b.dataset.chFilter) return; // credit-history modal filters are handled in dashboard.html
