  ...
  try {
    ...
    if (!snap.exists) {
      ...
      await userRef.set({
        ...
        freeCreditsGiven: 10000, totalGenerations: 0, createdAt: FieldValue.serverTimestamp(),
    ...
    if (data.balance === undefined) {
      ...
      if (!data.tokens_limit || data.tokens_limit < 1000) {
        newBalance = 10000;
        await userRef.update({ balance: 10000, freeCreditsGiven: 10000, totalPurchased: 0, totalUsed: 0, plan: data.plan||'free', cycle_start: data.cycle_start||new Date().toISOString(), tokens_limit: 10000, tokens_used: 0 });
        data.balance = 10000;
      } else {
    ...
    res.json({
      uid: decoded.uid, email: data.email, displayName: data.displayName, photoURL: data.photoURL||decoded.picture||'',
      plan: data.plan||'free', balance: remaining, limit: remaining + (data.totalUsed||0), used: data.totalUsed||data.tokens_used||0,
      remaining: Math.max(0, remaining), totalPurchased: data.totalPurchased||0, totalUsed: data.totalUsed||0,
      freeCreditsGiven: data.freeCreditsGiven||10000, validity_till: data.validity_till||null, totalGenerations: data.totalGenerations||0
    });
  } catch (err) {
